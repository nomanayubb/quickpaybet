...
Last compiled: 2026-09-07, by Claude Code. Originally written 2026-09-06 after a full line-by-line audit of the actual codebase (not just the chat history) plus live reproduction of bugs by running the code; kept current since as real work has landed, most recently the betting exchange, its admin dashboard, and cash-out (Section 4.11).
...
- **A separate, much larger feature the client has requested for later (not started):** an admin-configurable loss-cashback/rebate system — a percentage of a user's net losses credited to a separate balance, with per-user on/off and rate, an optional wagering-multiplier requirement before it's withdrawable, and admin control over pop-up-vs-auto-deduct at withdrawal time. Explicitly deferred to its own dedicated plan-and-approval pass (this is a different feature from exchange commission, and from anything currently built) — flagged as a background task suggestion in this session, not yet designed or scoped.
...
**Phase 5 — Betting Engine** — ✅ Mostly done, plus a full second betting mode added 2026-09-07: a Betfair-style back/lay exchange (`apps/exchange`) alongside the original fixed-odds single/parlay sportsbook — real order matching, exposure, commission, settlement, a read-only admin dashboard, and cash-out, all tested (Section 4.11). ⏳ Open: advanced exchange order types (low priority); the separate loss-cashback feature request (Section 8) has no design yet.
...
**Phase 11 — Testing & QA** — ✅ Mostly done. Tests exist, pass, and CI (GitHub Actions) runs them automatically on every push. 46 tests project-wide as of 2026-09-07 (18 foundation + 14 for exchange matching/settlement + 13 for cash-out + 1 for the withdrawal-vs-reserved-balance fix). ⏳ Open: test coverage for the bug classes found in Phase 0.5 (exception handling, permission boundaries) should keep growing as new features land.
